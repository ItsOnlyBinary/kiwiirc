<template>
    <!--
        .kiwi-messagelist is a layout-only wrapper — overflow is removed so it doesn't
        compete with VList's own internal scroll container.
        VList IS the scroller; it must have an explicit pixel height to measure its
        viewport. We achieve this by making the wrapper position:relative + height:100%
        and VList position:absolute inset:0 so it fills the space exactly.
    -->
    <div
    ...
    >
        ...
        <VList
            ref="vlist"
            v-slot="{ item }"
            :data="flatItems.items"
            :item-props="itemProps"
            :keep-mounted="activeSeparator !== null ? flatItems.embeds.concat(activeSeparator) : flatItems.embeds"
...
export default {
    ...
    computed: {
        ...
        /**
         * Flattened list for VList. Each entry is one of:
         *   { type: 'scrollback', key }           — load history button (always first if visible)
         *   { type: 'day',        key, date }      — date separator
         *   { type: 'unread',     key }             — unread marker
         *   { type: 'message',    key, message, isRepeat }
         *   { type: 'joining',    key }             — joining channel loader (last)
         *   { type: 'bufferkey',  key }             — channel key prompt (last)
         *
         * Putting everything here means VList virtualises the whole list including
         * header/footer UI — no DOM nodes exist outside the virtual list.
         */
        flatItems() {
            const items = [];
            const sticky = [];
            ...
            // Scrollback button at the top
            if (this.shouldShowChathistoryTools) {
                items.push({ type: 'scrollback', key: 'scrollback' });
            }

            const groups = this.filteredMessagesGroupedDay;
            const showDaySeparators = groups.length > 1;

            groups.forEach((day, gIdx) => {
                if (showDaySeparators && day.messages.length > 0) {
                    sticky.push(items.length);
                    items.push({
                        type: 'day',
                        key: `day-${day.dayNum}`,
                        date: day.messages[0].time,
                    });
                }
                ...
                day.messages.forEach((message, mIdx) => {
                    ...
                    items.push({
                        ...
                        gIdx,
                        mIdx,
                        isRepeat: this.isRepeat(gIdx, mIdx, message),
                    });
                });
            });
            ...
            return { items, sticky: sticky.reverse(), embeds };
        },
    ...
    methods: {
        ...
        onVListScroll() {
            ...
            this.activeSeparator = this.flatItems.sticky.find((index) => start >= index) || null;
        ...
        maybeScrollToId(id, position = 'middle') {
            ...
            const idx = this.flatItems.items.findIndex(
