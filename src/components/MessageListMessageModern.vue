...
const displayAvatar = (props) => {
    ...
    if (!props.message.user && !props.ml.buffer.state.setting('avatars.show_without_user')) {
        return false;
    }
...
const buildMessageTop = (props, context, cache) => {
    ...
    if (props.message.nick) {
        ...
        top.push(h('a', {
            ...
            'data-nick': cache.lcNick,
            'onMouseover': () => (context.emit('hoverNick', cache.lcNick)),
            'onMouseout': () => (context.emit('hoverNick', '')),
            style,
        }, [
            h('span', {
...
const messageModern = (props, context) => {
    ...
    return [h('div', {
        'class': {
            ...
            'kiwi-messagelist-message--hover': props.isHover,
            'kiwi-messagelist-message--unread': props.isUnread,
            'kiwi-messagelist-message--info-open': props.isInfoOpen,
            'kiwi-messagelist-message--blur': props.isBlur,
...
messageModern.props = ['ml', 'message', 'isUnread', 'isRepeat', 'isHover', 'isInfoOpen', 'isBlur'];
messageModern.emits = ['hoverNick'];
