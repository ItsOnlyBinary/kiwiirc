...
class State extends EventEmitter {
    ...
    #availableStartups = Object.create(null);

    constructor() {
        super();

        // Vue-style event aliases
        ...
        this.settings = configTemplates.default;
        this.user_settings = reactive({});
        this.ui = reactive({
            active_network: 0,
            active_buffer: '',
            last_active_buffers: [],
    ...
    setting(name, val) {
        if (typeof val !== 'undefined') {
            if (val === this.getSetting('settings.' + name)) {
                // Remove setting from user_settings if it's the default
                return this.setSetting('user_settings.' + name, null);
            }
            // Setting any setting always goes into the user's own settings space
        ...
        return typeof userSetting !== 'undefined' ?
            userSetting :
    ...
    exportState(includeBuffers) {
        ...
        if (includeBuffers) {
            toExport.networks = this.networks.map((network) => {
                let networkObj = {
                    id: network.id,
                    name: network.name,
                    connection: {
        ...
        toExport.user_settings = this.user_settings;
    ...
    importState(stateStr) {
        ...
        if (importObj && importObj.user_settings) {
            Object.keys(this.user_settings).forEach((k) => delete this.user_settings[k]);
            Object.assign(this.user_settings, importObj.user_settings);
        }
    }

    resetState() {
        Object.keys(this.user_settings).forEach((key) => {
            delete this.user_settings[key];
        });
        this.networks.splice(0, this.networks.length);
    ...
    getNetwork(networkid) {
        return _.find(this.networks, { id: networkid });
    }

    getNetworkFromAddress(netAddr) {
        return _.find(this.networks, (net) => netAddr.toLowerCase() === net.connection.server.toLowerCase());
    }
    ...
    addNetwork(name, nick, serverInfo) {
        ...
        let network = new NetworkState(networkid, this, this.#userDict, this.#bufferDict);
        network.name = name;
        ...
        // Add the server buffer
        this.addBuffer(network.id, '*').joined = true;

        this.$emit('network.new', { network });
    ...
    removeNetwork(networkid) {
        ...
        let findNewNetwork = network === this.getActiveNetwork();
        ...
        this.$emit('network.removed', { network });
    }
    ...
    openLastActiveBuffer() {
        ...
        if (!targetBuffer) {
            ...
            if (network) {
                ...
                targetBuffer = buffer || network.serverBuffer();
            }
        }
    ...
    getBufferByName(networkid, bufferName) {
        ...
        let toMatch = bufferName.toLowerCase();
        return _.find(network.buffers, (b) => b.name.toLowerCase() === toMatch);
    }
    ...
    addBuffer(networkid, bufferName) {
        ...
        // Make sure we at least have this network
        let network = this.getNetwork(networkid);
        if (!network) {
            return false;
        }

        buffer = new BufferState(bufferName, network.id, this, this.#messages);
        network.buffers.push(buffer);

        this.$emit('buffer.new', { buffer });
    ...
    removeBuffer(buffer) {
        ...
        if (buffer.isQuery()) {
            let remainingBuffers = this.getBuffersWithUser(network.id, buffer.name);
            if (remainingBuffers.length === 0) {
                // BUG FIX: was network.d (typo) in original
                this.removeUser(network.id, { nick: buffer.name });
            }
        }
    ...
    addMessage(buffer, message) {
        ...
        let typesForActivity = ['privmsg', 'action', 'notice', 'wallops'];
        if (buffer.setting('traffic_as_activity') && message.type === 'traffic') {
            typesForActivity.push('traffic');
        }

        if (typesForActivity.indexOf(message.type) > -1) {
        ...
        let extraHighlights = (this.setting('highlights') || '').toLowerCase().split(' ');
        ...
        if (!buffer.isRaw() && this.setting('teamHighlights')) {
        ...
        if (
            ...
            if (notifyTitle) {
                this.$emit('notification.show', notifyMessage, {
                    title: notifyTitle,
                    onclick: () => {
                        this.setActiveBuffer(buffer.networkid, buffer.name);
    ...
    // Modify a network's user array without hitting Vue's reactive system until fn()
    ...
    removeUser(networkid, user) {
        ...
        let buffers = this.getBuffersWithUser(networkid, user.nick);
        buffers.forEach((buffer) => {
            this.removeUserFromBuffer(buffer, user.nick);
        });
    ...
    addMultipleUsersToBuffer(buffer, newUsers) {
        ...
        newUsers.forEach((newUser) => {
            ...
            let userObj = this.getUser(network.id, user.nick);
    ...
    changeUserNick(networkid, oldNick, newNick) {
        ...
        let user = this.getUser(network.id, oldNick);
        if (!user) {
            return;
        }
    ...
    getStartups() {
        return this.#availableStartups;
    }
}

let instance = null;
export default function singleton() {
    instance = instance || new State();
    return instance;
}
