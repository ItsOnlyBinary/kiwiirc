        ...
        <div class="kiwi-welcome-simple-section kiwi-welcome-simple-section-connection">
            <template v-if="!network || network.state === 'disconnected'">
                <form @submit.prevent="formSubmit" class="u-form kiwi-welcome-simple-form">
                    ...
                    <input type="radio" id="gender_u" value="U" v-model="gender" checked="checked">
                    <label for="gender_u">Autre</label>
                    <input-text class="kiwi-welcome-simple-location" :label="$t('location')" v-model="location" /><input-text v-if="showChannel" class="kiwi-welcome-simple-channel" :label="$t('channel')" v-model="channel" />
                    <button
...
export default {
    ...
    created: function created() {
        ...
        if ((options.autoConnect || Misc.queryStringVal('auto') === 'true') && this.nick && this.channel) {
            this.startUp();
        }
    },
};
