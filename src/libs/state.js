...
function createNewState() {
    ...
    const methods = {
        ...
        importState(stateStr) {
            ...
            if (importObj && importObj.networks) {
                ...
                importObj.networks.forEach((importNetwork) => {
                    let network = reactive(new NetworkState(
                        importNetwork.id,
                        state,
                        userDict,
                        bufferDict
                    ...
                    importNetwork.buffers.forEach((impBuffer) => {
                        let buffer = reactive(new BufferState(
                            impBuffer.name,
                            network.id,
                            state,
                            messages
        ...
        addNetwork(name, nick, serverInfo) {
            ...
            let network = reactive(new NetworkState(networkid, state, userDict, bufferDict));
            network.name = name;
        ...
        addBuffer(networkid, bufferName) {
            ...
            buffer = reactive(new BufferState(bufferName, network.id, state, messages));
            network.buffers.push(buffer);
    ...
    Object.assign(state, stateObj, methods, {
        $on: state.on,
        $off: state.off,
        $once: state.once,
        $emit: state.emit,
